...
export const Card = ({img, name, time, desc, read, views, comments, color, textColor}: ICard) => {
    ...
    return(
        <div className="w-1/4 rounded-3xl flex flex-col justify-between items-center text-center gap-2 bg-white h-[500px] hover:scale-110 ease-in-out transition w-80">
            ...
            <p className={`text-xs ${textColor}`}>{time} ago</p>
            <p className="text-2xl"><b>{name}</b></p>
            <div className="overflow-auto">
                <p className="text-sm py-4">{desc}</p>
            </div>
            <div className={`flex justify-around w-full  rounded-b-2xl border border-black ${color} text-white`}>
